...
> 控制台新建服务默认使用 `visitor`（凭证访问），不开放公网业务端口；公开服务需明确选择 `public`。为兼容已有 API 客户端，创建 API 省略 `mode` 时仍使用 `public`。需要来源 IP 临时放行时选择 `spa`；需要凭证化访问且不希望监听公网业务口时选择 `visitor`。
...
UMBRA_TAG=0.2.0 docker compose -f deploy/compose.gate.yml up -d
...
控制台首页是「节点」。点击节点直接查看它的服务，在节点内添加服务会自动选中当前节点。填写它可访问的目标地址与端口，再选择凭证访问、临时放行或公开访问。「全部服务」和快捷搜索用于跨节点查找。限速和超时在高级设置中。保存后直接进入连接面板：

- “配置就绪”表示 Node 已确认最新配置，不代表目标服务或外网路径已验证。
...
- 导出 JSON 带 `schemaVersion`。节点只包含名称、备注等可复用字段；服务包含协议、公网端口、内网地址、访问模式、启停、白名单、限速、连接数和超时。
- **不会**导出节点认证凭证、证书私钥、管理端认证信息、访问票据或运行状态。来源标识用于重复导入识别，不会把源入口的数据库 ID 当成目标入口的实体 ID。
...
在需要访问内网服务的电脑上安装对应平台的 `umbra-visit`。在控制台「服务」中打开凭证访问服务的「连接」面板，选择「签发 24 小时访问命令」，然后执行只显示一次的命令：
...
生产环境建议固定使用当前正式版本 `0.2.0`，避免 `latest` 更新后意外改变运行版本。
...
# 把入口的 ca.crt 放到当前目录
UMBRA_TAG=0.2.0 docker compose -f deploy/compose.node.yml up -d
```
...
- 控制台新建服务默认为 `visitor`；API 省略 `mode` 时仍默认为 `public`。公网上线前，请确认访问模式、CIDR 规则、目标地址和业务自身的认证配置。
