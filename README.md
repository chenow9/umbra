...
生产环境建议固定使用当前正式版本 `0.3.1`，避免 `latest` 更新后意外改变运行版本。
...
- 临时放行的内核丢弃需要 Linux、nftables 和 `CAP_NET_ADMIN`，当前只针对 IPv4。来自 IPv6 的连接只在用户态拒绝：TCP 会完成握手后被关闭，UDP 会收到并丢弃，因此 IPv6 扫描仍能发现端口。若入口主机有公网 IPv6 地址而你依赖“不可发现”，请用 `-bind` 只监听 IPv4 地址，或在主机防火墙上阻断 IPv6 到服务端口的流量。不满足内核丢弃条件时同样回退为用户态拒绝；即使使用内核丢弃，也不应理解为“绝对不可发现”。
...
- 保护并备份整个 `-tls-dir`：其中包含入口证书与私钥（`gate.crt` / `gate.key`）、CA 证书、管理员口令哈希、TOTP 密钥、节点凭证、服务配置和流量历史。泄露备份等同于泄露 TOTP 密钥，并允许离线猜测管理员口令；不要将该目录上传到仓库或传给不可信第三方。
- CA 私钥不会落盘：首次启动时生成 CA 并签发入口证书后，CA 私钥即被丢弃，磁盘上只有 `ca.crt`。这意味着无法用同一 CA 再签发新的入口证书；`gate.key` 泄露或入口证书（有效期 10 年）到期时，需要删除 `ca.crt`、`gate.crt`、`gate.key` 后重启入口生成新 CA，并把新的 `ca.crt` 重新下发到所有节点和访问端（它们通过 `--tls-ca` 固定信任该 CA）。请把这一步纳入应急预案。
- 登录限流是按 IP 的（15 分钟 8 次失败）加一个跨 IP 的全局退避（最长 60 秒）。全局退避只作用于近期失败过的 IP，未失败过的 IP 不受影响；但如果 `-http-trust-proxy` 配置错误导致所有客户端都表现为同一个来源 IP，任意用户的失败都会消耗管理员的额度并把管理员一起锁住。限流状态只在内存中，必要时重启进程即可清零；不要因此关闭 2FA。
