...
- 完整的密码 + 第二因素成功后清零；
- 密码和第二因素失败都计入统一预算；
- 全局退避只对「窗口内有失败或在途尝试记录」的 IP 生效：未失败过的 IP 在退避期间照常放行，失败一次后才进入退避。攻击者从少数 IP 持续喂错口令不能把管理员从干净 IP 锁在外面；分布式猜测仍受每 IP 8 次/15 分钟和 2 路并行 scrypt 的约束；
- 限流状态只在内存，进程重启即清零；若管理员自己的 IP 也进入了 60 秒退避，可重启进程或走本机 `reset-2fa`，不得因此自动关闭 2FA。
