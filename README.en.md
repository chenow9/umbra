...
Pass the credential through the `UMBRA_TOKEN` environment variable or `--token-file <path>`. `--token` still works but shows up in the process arguments, where other local users can read it with `ps`. None of the install commands generated by the console put the credential on a service or container command line.
