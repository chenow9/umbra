...
UMBRA_TAG=0.3.1 docker compose -f deploy/compose.gate.yml up -d
...
Pin production deployments to the current stable version, `0.3.1`, so a future `latest` update cannot change the running version unexpectedly.
...
UMBRA_TAG=0.3.1 docker compose -f deploy/compose.node.yml up -d
```
