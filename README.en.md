...
UMBRA_TAG=0.3.2 docker compose -f deploy/compose.gate.yml up -d
...
- `traffic` — rate-curve samples (written about once a minute, flushed in full on clean shutdown)
...
Console-generated node installation commands use `--token` by default, exposing the credential in process arguments. Set `--hide-node-token` on `umbrad` or `UMBRA_HIDE_NODE_TOKEN=true` to hide it (default: `false`; the command-line flag takes precedence). When enabled, Windows / Docker use protected credential files with `--token-file`; Linux / macOS and locally spawned nodes use environment variables. Restart the gateway after changing the setting. It applies to newly generated installation commands and subsequent local launches; reinstall existing nodes with the new commands to apply it. Manual launches still support `--token`, `--token-file <path>`, and `UMBRA_TOKEN`.
...
The binary install command generated by the console registers this node as a system service. One machine can run several nodes: they share one `umbra-node` binary and each has its own service name, credential, and CA. The service name comes from the node id, with underscores written as hyphens. Node `nde_ab12` is the Linux service `umbra-node-nde-ab12`. Below, `NODE` stands for that piece; use the name from the copied command. You can close the terminal after installation; the node keeps running and starts automatically with the host. Running the same node's command again replaces only that node.

An install created by an older command still uses the fixed name `umbra-node` (`io.umbra.node` on macOS, `UmbraNode` on Windows, container `umbra-node` for Docker). The new command leaves those in place. After the new service is running, stop the old one so two processes do not keep the same enrollment:
...
Remove this Linux node completely:
...
# Remove the shared binary only when no other Umbra node remains on this machine
...
Remove this macOS node completely:
...
Remove this Windows node completely:
...
Nodes on the same machine share `%ProgramFiles%\Umbra\umbra-node.exe`. Delete that file only when no other Umbra node remains.

Removing one node keeps the others' configuration. When permanently retiring a node, revoke its credential in the console first, then remove that node's own configuration directory. Removing local files does not remove the node record from the console.
...
Pin production deployments to the current stable version, `0.3.2`, so a future `latest` update cannot change the running version unexpectedly.
...
The enroll dialog's Docker command writes the CA locally and runs `docker run --network host`. The container name, CA, and credential are per node, so running another node's command does not remove the previous container. Compose still works:

```bash
cp deploy/node.env.example node.env   # set UMBRA_SERVER / UMBRA_TOKEN
# copy the gate's ca.crt into the current directory
UMBRA_TAG=0.3.2 docker compose -f deploy/compose.node.yml up -d
```
...
Node: `--server`, `--tls-ca`, `--token-file` (or the `UMBRA_TOKEN` environment variable). The visitor client also needs `--ticket` and `--local`.
...
- Kernel-level drops require Linux, nftables, and `CAP_NET_ADMIN`, and currently protect IPv4 only. Connections arriving over IPv6 are rejected in user space: TCP completes the handshake before being closed and UDP is received then discarded, so an IPv6 scan can still find the port. If the gateway host has a public IPv6 address and you rely on scan resistance, bind entries to an IPv4 address with `-bind` or block IPv6 to the service ports in the host firewall. When the kernel-drop requirements are not met Umbra falls back to user-space rejection in the same way. Kernel drops should be understood as scan resistance, not guaranteed invisibility.
...
- Protect and back up the entire `-tls-dir`. It contains the gateway certificate and private key (`gate.crt` / `gate.key`), the CA certificate, administrator password hash, TOTP secret, node credentials, service settings, and traffic history. A leaked backup exposes the TOTP secret and enables offline password guessing; never commit or share it with an untrusted party.
- The CA private key is never written to disk. On first start the gateway generates a CA, signs its own certificate, and discards the CA key; only `ca.crt` remains. Consequently no further certificate can be issued under that CA: if `gate.key` leaks or the gateway certificate (valid for 10 years) expires, delete `ca.crt`, `gate.crt` and `gate.key`, restart the gateway to generate a new CA, and redistribute the new `ca.crt` to every node and visitor client (they pin it via `--tls-ca`). Plan for this in your incident procedures.
- Login rate limiting is per IP (8 failures per 15 minutes) plus a cross-IP global backoff (up to 60 seconds). The global backoff applies only to addresses that have failed recently, so a clean address is not affected; however, if `-http-trust-proxy` is misconfigured so that every client appears to come from one source address, anyone's failures consume the administrator's budget and lock the administrator out together with them. Rate-limit state is in memory only; restart the process to clear it if necessary, and do not disable 2FA in response.
